<?php
    ...
    if (empty($productos)) { 
        header('Location: ../venta_sucursales.php?incorrecto=1');
    }

    $insertar_venta = "INSERT INTO envio_info (total, fecha, hora) VALUES ('$total','$fecha','$hora')";
    $query = mysqli_query($conexion,$insertar_venta);

    $consultar_ultimo_registro = "SELECT id FROM envio_info ORDER BY ID DESC LIMIT 1";
    ...
    if (!empty($productos) && is_array($productos) ) { 
        for($i = 0; $i < count($productos); $i++){
            $insertar_producto = "INSERT INTO envios_productos (id,nombre, cantidad, precio, importe) VALUES ('$id_ultima_venta','$productos[$i]','$cantidades[$i]','$precios[$i]','$importes[$i]')";
